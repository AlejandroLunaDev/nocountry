...
import { MessageSquare, Users, Calendar, MapPin, ArrowLeft } from "lucide-react"
import { teams } from "@/lib/data"
import type { TeamMember } from "@/lib/types"
import { HexagonalLayout } from "@/components/hexagonal-layout"
import Link from "next/link"

export default function HexagonalView() {
    ...
    <div className="min-h-screen bg-[#050A1A] text-white">
      ...
      <main className="container mx-auto py-8 px-4">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold">Vista Hexagonal de Equipos</h1>
          <Link href="/">
        ...
        <Tabs defaultValue="team1" value={selectedTeam} onValueChange={setSelectedTeam} className="w-full">
            ...
            <TabsContent key={team.id} value={team.id} className="space-y-8">
              <div className="flex flex-col gap-6">
                <div className="flex justify-between items-center">
                  ...
                  <Button onClick={() => setIsDialogOpen(true)} className="bg-[#FF2D8A] hover:bg-[#D91A70] text-white">
                    Ver Insights
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                  <div className="bg-[#0A1428] border border-[#1A2035] rounded-lg p-4 flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold">{team.insights.messages}</p>
                    </div>
                  </div>
                  <div className="bg-[#0A1428] border border-[#1A2035] rounded-lg p-4 flex items-center gap-4">
                    <div className="bg-[#1A2035] p-3 rounded-full">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold">{team.insights.participation}%</p>
                    </div>
                  </div>
                  <div className="bg-[#0A1428] border border-[#1A2035] rounded-lg p-4 flex items-center gap-4">
                    <div className="bg-[#1A2035] p-3 rounded-full">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold">{team.insights.attendance}%</p>
                    </div>
                  </div>
                </div>

                {/* Hexagonal Layout */}
                <HexagonalLayout members={team.members} onMemberClick={openInsightsDialog} />
              </div>
            </TabsContent>
          ))}
        </Tabs>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="bg-[#0A1428] border border-[#1A2035] text-white max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-400">Selecciona hasta 5 equipos para contratar</DialogDescription>
          </DialogHeader>
            ...
            <div className="bg-[#050A1A] rounded-lg p-4 mb-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[#FF2D8A] font-medium">{selectedMember.role}</p>
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-4">
                    ...
                    <div>
                      ...
                      <p className="font-bold">{selectedMember.experience} años</p>
                    </div>
                    <div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {selectedMember.skills.map((skill, index) => (
                        <span key={index} className="text-xs py-1 px-2 rounded-full bg-[#1A2035] text-white">
                          {skill}
                        </span>
                      ))}
                    </div>
