    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-[#0A1428] border border-[#1A2035] text-white max-w-4xl p-0 overflow-hidden">
          ...
          <div className="relative p-6">
            <DialogHeader className="mb-6">
              ...
              <DialogDescription className="text-gray-400">
                Analiza el rendimiento y selecciona hasta 5 equipos para contratar
              </DialogDescription>
            </DialogHeader>

            <Tabs
              value={activeTab}
              onValueChange={(value) => setActiveTab(value as "profile" | "teams")}
              className="w-full"
            >
                      ...
                      <div className="md:w-1/3">
                        <div className="relative">
                          <div className="w-full aspect-square rounded-lg overflow-hidden border-2 border-[#1A2035] relative">
                            <Image
                              src={selectedMember.avatar || "/placeholder.svg?height=300&width=300"}
                        ...
                        <div className="mt-4 space-y-3">
                          <div className="bg-[#0A1428] rounded-lg p-3">
                            ...
                            <p className="font-bold text-lg">{selectedMember.age} años</p>
                          </div>
                          <div className="bg-[#0A1428] rounded-lg p-3">
                            <p className="text-sm text-gray-400">Experiencia</p>
                            <p className="font-bold text-lg">{selectedMember.experience} años</p>
                          </div>
                          <div className="bg-[#0A1428] rounded-lg p-3">
                            <p className="text-sm text-gray-400">Inglés</p>
                            <p className="font-bold text-lg">{selectedMember.english}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="md:w-2/3">
                        <div className="flex flex-col h-full">
                          <div>
                            ...
                            <p className="text-[#FF2D8A] font-medium text-lg">{selectedMember.role}</p>
                          ...
                          <div className="mt-6 flex-1">
                            <h4 className="font-medium mb-3">Estadísticas de rendimiento</h4>
                            <div className="space-y-4">
                              <div className="space-y-2">
                                <div className="flex justify-between">
                                  <span className="text-gray-400">Participación</span>
                                  <span className="font-medium">{selectedMember.stats.participation}%</span>
                                </div>
                                <Progress value={selectedMember.stats.participation} className="h-2 bg-[#1A2035]">
                              ...
                              <div className="space-y-2">
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-medium">{selectedMember.stats.attendance}%</span>
                                </div>
                                <Progress value={selectedMember.stats.attendance} className="h-2 bg-[#1A2035]">
                              ...
                              <div className="space-y-2">
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-medium">
                                    {Math.min(100, Math.floor(selectedMember.stats.messages / 1.5))}%
                                  </span>
                                </div>
                                <Progress
                                  value={Math.min(100, Math.floor(selectedMember.stats.messages / 1.5))}
                              ...
                              <div className="space-y-2">
                                <div className="flex justify-between">
                                  <span className="text-gray-400">Calidad de código</span>
                                  <span className="font-medium">{70 + Math.floor(Math.random() * 25)}%</span>
                                </div>
                                <Progress value={70 + Math.floor(Math.random() * 25)} className="h-2 bg-[#1A2035]">
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className="flex justify-between items-center">
                              ...
                              <Badge variant="outline" className="bg-[#1A2035] border-none">
                            ...
                            <div className="flex items-center gap-4 mt-2 text-sm">
                              <div className="flex items-center">
                                ...
                                <span>{team.insights.attendance}% asistencia</span>
                              </div>
                              <div className="flex items-center">
                                <div className="w-2 h-2 rounded-full bg-[#00D1FF] mr-1"></div>
                                <span>{team.insights.participation}% participación</span>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <p className="font-bold text-2xl">{selectedTeams.length}</p>
                          ...
                          {Array.from({ length: 5 }).map((_, i) => (
                            <div
                              ...
                              className={`w-8 h-1 rounded-full ${
                                i < selectedTeams.length ? "bg-[#FF2D8A]" : "bg-[#1A2035]"
                              }`}
                            ></div>
                          ))}
                        </div>
                      ...
                      <div className="text-right">
                        ...
                        <p className="font-bold text-3xl">${totalPrice.toLocaleString()}</p>
                        <p className="text-xs text-gray-400 mt-1">$1,500 por equipo</p>
                      </div>
                    </div>
                  </div>
                </motion.div>
        ...
        <DialogFooter className="bg-[#050A1A] p-4 border-t border-[#1A2035]">
          <Button variant="outline" onClick={onClose} className="border-[#1A2035] text-white hover:bg-[#1A2035]">
            Cancelar
          </Button>
          <Button className="bg-[#FF2D8A] hover:bg-[#D91A70] text-white" disabled={selectedTeams.length === 0}>
